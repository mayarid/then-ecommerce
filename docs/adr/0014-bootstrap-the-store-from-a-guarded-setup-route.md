...
Superseded by [ADR-0016](./0016-open-setup-until-the-store-has-an-administrator.md). The `/setup` route remains. The `SETUP_TOKEN` guard does not. The `BETTER_AUTH_SECRET` consequence below is superseded by [ADR-0017](./0017-generate-the-auth-secret-on-first-use.md).
